...
Options let you change certain aspects of the game. They are applied once as the translation is loaded at runtime using [`Game.ChangeTranslation](Game#gamechangetranslation). All options begin with the comment opening and a pound sign: `//#` - followed by `OPTION=VALUE` kind of string where "OPTION" is the corresponding name. Following options are supported:
- **GameName** - this option is used for matching this translation file with the game. If it has a value set, then the game will only load this translation if it has the same name. Editor should fill this option when creating a translation file.
- **UniqueID** - this option is used for matching this translation file with the game. If it has a value set, then the game will only load this translation if it has the internal ID number. Editor should fill this option when creating a translation file. It's recommended to not change this option by hand.
